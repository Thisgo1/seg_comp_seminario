...
;
;
const useProductDetails = (provider, id)=>{
        ...
        const fetchData = async ()=>{
            try {
                ...
                const [allProducts] = await Promise.all([
                    __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$services$2f$api$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["api"].providers.getAll().catch(()=>[])
                ]);
                const foundProduct = allProducts.find((p)=>p.id === id && p.provider === provider);
                if (!foundProduct) {
                    setIsMock(true);
                    ...
                    setRelatedProducts(allProducts.filter((p)=>p.id !== id).slice(0, 4).map((p)=>({
                            ...p,
                    ...
                    setRelatedProducts(allProducts.filter((p)=>p.id !== id && p.category === foundProduct.category).slice(0, 4));
                }
            } catch (err) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__da2bcc19._.js.map